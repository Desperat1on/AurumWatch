...
- **追加（2026-09-14，用户要求）**：主窗口与任务栏图标原先还是 Tk 自带的羽毛，用户看过之后定了「一并换成金币」。于是 `tools/icon.py` 挪进包里（`aurumwatch/icon.py`，构建脚本改从包里引），新增 `png_bytes`（同样是标准库拼 PNG），主窗口开窗时现画一张 64×64 交给 `iconphoto(True, …)`——不落盘，写不进去的目录里也照样有图标；换不上就当没这回事，一个图标不该拦住整个窗口（返回值要留着：Tk 图像没人引用就会被回收）。设置窗口是之后才开的 Toplevel，`-default` 让它共用同一个类图标。**实测**：源码运行与打包产物上，标题栏都已是金币；`GCLP_HICON`／`GCLP_HICONSM` 两个句柄画出来都是金币（此前是羽毛），设置窗口报的是同一个类图标。**未拍到的**：任务栏本身（锁屏下桌面不在，截不到；Windows 的任务栏按钮用的就是那个 `GCLP_HICON`）。新增 `tests/test_icon.py` 5 个用例锁形状（ICO 七档的表头与位图头、PNG 四角透/币面暖色、同一形状两次画出来字节一致、只用标准库），全套 224 → 229。
